 ...
 * 作者：张津阳
 * 日期：2018年2月1日
 * 备注：目前的问题就是，设置 泵A 和 泵B 的采样时间并不是独立的，而是一起改动的
 ...
 *       （5）那么，已经实现了通过Qt上位机设置蠕动泵的采样时间（也就是定时器的触发间隔时间）
 */
...
void loop() {
  ...
  else
  {
    ...
    if( strPart == "ST" )
    {
      Serial.print("samplingTime change to ");
      Serial.print(numberPart);
      Serial.println("ms !");
      
      samplingTime = numberPart;
      FlexiTimer2::set( samplingTime, Pump_stop );
