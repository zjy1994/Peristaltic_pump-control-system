 ...
 * 作者：张津阳
 * 日期：2018年2月5日
 * 备注：改成Arduino Uno可以跑的程序，主要是改定时器
 ...
 *       （9）增加修改系统启动延时的时长，这个也已经实现了
 */
//#include<FlexiTimer2.h>
#include <MsTimer2.h>
const long T = 500;   //脉冲信号的周期，单位：微秒，对应的脉冲信号的频率就是2000Hz
...
int systemStartDelayTime = 30;
boolean startDelay = false;      //是否需要开机延时
...
void PumpA_Action()
{
    ...
    MsTimer2::set( samplingTime_PumpA, PumpA_stop );
...
void PumpB_Action()
{
    ...
    MsTimer2::set( samplingTime_PumpB, PumpB_stop );
...
void delay_s(int delayTime)  //秒级延时
{
...
void setup() {
    ...
    //delay(5000);
    //Serial.write("Waiting for your command...");
}
...
void loop() {
  ...
  if( strCommand != "" )
  {
    ...
    if( commaPosition == -1 )    //代表字符串里面不含逗号','
    {
      if( strCommand == "a" )
      {
        PumpA_Action();
      ...
      else if( strCommand == "Activate" )
      {
        ...
        Serial.println("Please enter your order:");
        strCommand = "";
        }
      else
      {
        Serial.print("Your command order: ");
        Serial.println( strCommand );

        if( startDelay == true )    //系统需要开机延时
        {
          Serial.print("System needs to delay ");
          Serial.print(systemStartDelayTime);
          Serial.println("s .");
          delay_s( systemStartDelayTime );
          Serial.println("The system's delay is over!");
          }
    ...
    else
    {
      ...
      if( strPart == "PAST" )    //蠕动泵A要改变进样时间
      {
       ...
       if( strPart == "PBST" )    //蠕动泵B要改变进样时间
       {
        ...
        if( strPart == "SysStartDelay" )
        {
          if( numberPart == 1 )
          {
            Serial.println("Start the delay function!");
            startDelay = true;
            }
          else{
            Serial.println("Turn off the delay function!");
            startDelay = false;
            }
          }
        if( strPart == "SSDT" )
        {
          Serial.print("The system delay time is set to ");
          Serial.print(numberPart);
          Serial.println("s !");
  
          systemStartDelayTime = numberPart;          
          }
